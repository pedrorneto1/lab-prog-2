...
public class Jogo {
	...
	public void rodada() throws EstrategiaInvalidaException, JogadaInvalidaException {
		...
		// Se ambos passaram, realiza-se o desempate pelo menor número de 
		// peças, caso continue empate, verifica-se a menor soma dentre as
		// peças na mão de cada jogador. Caso continue igual, define o empate.
		if (jogadaJ1.getTipo() == TipoJogada.PASSA && jogadaJ2.getTipo() == TipoJogada.PASSA) {
			if (jogador1.getNumPecas() < jogador2.getNumPecas()) {
				...
				this.vencedor = this.jogador2.getNome();
				this.jogadorVencedor = jogador2;
			} else if (jogador1.getSomaNumerosMao() < jogador2.getSomaNumerosMao()) {
				this.finalizado = true;
				this.vencedor = this.jogador1.getNome();
				this.jogadorVencedor = jogador1;
			} else if (jogador2.getSomaNumerosMao() < jogador1.getSomaNumerosMao()) {
				this.finalizado = true;
				this.vencedor = this.jogador2.getNome();
				this.jogadorVencedor = jogador2;
			} else {
...
//		System.out.println("==> final: " + (venceu == null ? "EMPATE" : venceu + " VENCEU") + "\n");
	...
	/**
	 * Verifica se a última peça jogada pelo vencedor é uma carroça.
	 * 
	 * @return Representação booleana que diz se é carroça ou não.
	 */
	public boolean verificaCarroca() {
		boolean eCarroca = false;
		if (this.jogadorVencedor.getUltimaJogada().getPeca() != null) {
			eCarroca = (this.jogadorVencedor.getUltimaJogada().getPeca().getNumDireito() == this.jogadorVencedor.getUltimaJogada().getPeca().getNumEsquerdo());
		}
		return eCarroca;
	}
	
	/**
	 * Verifica se a batida foi de lá e lô.
	 * 
	 * @return Representação booleana que diz se a batida foi de lá e lô.
	 */
	public boolean verificaLaeLo() {
		boolean eLaeLo = false;
		if (this.jogadorVencedor.getUltimaJogada().getPeca() != null) {
			eLaeLo = (this.jogadorVencedor.getUltimaJogada().getPeca().getNumDireito() == this.mesa.getNumNaEsquerda())
					|| (this.jogadorVencedor.getUltimaJogada().getPeca().getNumEsquerdo() == this.mesa.getNumNaDireita());
		}
		return eLaeLo;
	}
